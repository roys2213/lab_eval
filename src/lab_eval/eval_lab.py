      ...
      if type( obj ) != type( ref_obj ):
        ...
        if isinstance( obj, dict ):
          ...
          for i in range( max( len( obj ), len( ref_obj ) ) ):
            ...
            self.compare( obj[ i ], ref_obj[ i ], score )
    return score
...
def cli():
  ...
  ```    
  eval_single_lab  --conf ./lab_eval/examples/lab_caesar.cfg  student_dir
  ```

  The command below renames the log file with that 
  contains the evaluation:
  ```
  eval_single_lab  --conf ./lab_eval/examples/lab_caesar.cfg  -lab_id student_name student_dir
  ...
  ref_dir = os.path.abspath( os.path.expanduser( \
              config[ 'Instructor' ][ 'ref_dir' ] ) ) 
  if args.json_score_list is not None:
    json_score_list = os.path.abspath( os.path.expanduser(\
  ...
  lab_dir = os.path.abspath( os.path.expanduser( \
              args.lab_dir[ 0 ] ) )
